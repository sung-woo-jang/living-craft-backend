...
export class UsersController {
  ...
  async getMyProfile(@CurrentUser() user: any): Promise<SuccessBaseResponseDto<UserResponseDto>> {
    ...
    return new SuccessBaseResponseDto('사용자 정보를 조회했습니다.', responseData);
  }
  ...
  async updateMyProfile(
    ...
    return new SuccessBaseResponseDto('사용자 정보를 수정했습니다.', responseData);
  }
  ...
  async getCustomers(
    ...
    const { users, meta } = await this.usersService.findCustomers(paginationDto);
    
    const responseData = users.map(user => new UserResponseDto(user));

    return new PaginatedResponseDto('고객 목록을 조회했습니다.', responseData, meta);
  }
  ...
  async getUserById(@Param('id', ParseIntPipe) id: number): Promise<SuccessBaseResponseDto<UserResponseDto>> {
  ...
  async deactivateUser(@Param('id', ParseIntPipe) id: number): Promise<SuccessBaseResponseDto<UserResponseDto>> {
    const user = await this.usersService.deactivate(id);
    const responseData = new UserResponseDto(user);

    return new SuccessBaseResponseDto('사용자를 비활성화했습니다.', responseData);
  }
  ...
  async activateUser(@Param('id', ParseIntPipe) id: number): Promise<SuccessBaseResponseDto<UserResponseDto>> {
