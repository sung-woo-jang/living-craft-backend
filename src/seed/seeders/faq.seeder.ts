...
export class FaqSeeder {
    ...
    const additionalFaqs = [
      {
        ...
        answer:
          '토요일은 오전 9시부터 오후 3시까지 서비스 가능합니다. 일요일은 휴무이며, 공휴일은 별도 문의 바랍니다.',
        category: '서비스',
      },
      {
        question: '청소 범위는 어디까지인가요?',
        answer:
          '일반 청소는 거실, 침실, 화장실, 주방이 기본 범위입니다. 베란다나 다락방 등 추가 공간은 별도 협의 가능합니다.',
        category: '서비스',
      },
      {
        question: '몇 시간 정도 걸리나요?',
        answer:
          '서비스별로 다르지만 일반 청소는 2시간, 대청소는 3시간 정도 소요됩니다. 집 상태에 따라 시간이 달라질 수 있습니다.',
        category: '서비스',
      },
      {
        question: '정기 청소 할인이 있나요?',
        answer:
          '월 2회 이상 정기 예약 시 10% 할인, 주 1회 정기 예약 시 15% 할인 혜택을 드립니다.',
        category: '결제',
      },
      {
        question: '손해 보상은 어떻게 되나요?',
        answer:
          '작업 중 발생한 손해에 대해서는 보험을 통해 보상해드립니다. 단, 고가의 물품은 사전에 말씀해 주세요.',
        category: '서비스',
      },
    ];
  ...
  private async createFaqIfNotExists(
    ...
    if (!existing) {
      ...
      console.log(
        `⚠️  FAQ already exists: ${faqData.question.substring(0, 30)}...`,
      );
    }
  }
  ...
  async run(dataSource: DataSource): Promise<void> {
    ...
    for (
      let i = existingFaqsCount;
      i < existingFaqsCount + baseFaqsToCreate;
      i++
    ) {
