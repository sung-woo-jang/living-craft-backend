  ...
  async run(dataSource: DataSource, factoryManager: SeederFactoryManager): Promise<any> {
    ...
    if (reservationsToCreate > 0) {
      try {
        ...
        console.log('⚠️ Some reservations had duplicate codes, creating individually...');
    ...
    for (let i = 0; i < additionalReservationsCount; i++) {
      try {
        ...
        // 중복 예약번호 에러 무시
        console.log(`⚠️ Skipped one additional reservation due to duplicate code`);
      }
    }
    
    console.log(`✅ Attempted to create ${additionalReservationsCount} additional reservations`);
