...
export default class ReviewSeeder implements Seeder {
  async run(dataSource: DataSource, factoryManager: SeederFactoryManager): Promise<any> {
    ...
    console.log(`Found ${completedReservationsWithoutReview.length} completed reservations without reviews`);
    ...
    for (const reservation of reservationsToReview) {
      try {
        ...
        console.log(`✅ Review created for reservation ${reservation.reservationCode}`);
      } catch (error) {
        console.log(`⚠️ Failed to create review for reservation ${reservation.reservationCode}: ${error.message}`);
      }
    }
    ...
    if (reviewsToCreate > 0) {
      ...
      for (const reservation of availableReservations) {
        try {
          ...
          console.log(`⚠️ Failed to create additional review: ${error.message}`);
        }
      }
    ...
    console.log(`✅ Created ${additionalAvailableReservations.length} random reviews`);
